...
class benchmark
{
    ...
    public:
        ...
        void test(dllist *, int, int, int, bool);
        benchmark(int, bool);
        ~benchmark();
};
...
void benchmark::test(dllist * lista, int initial,int steps, int repetitions, bool Debug_Mode)
    ...
    for(i=0; i<steps; i++)
    {
        ...
        for(j=0;j<repetitions;j++)
        {
            ...
            for(k=0; k<amount_of_data; k++)
            {
                ...
                lista->append(tmp); //przypisanie okreslonej ilosci danych do listy
            }
