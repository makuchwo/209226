...
class benchmark
{
    ...
    public:
        ...
        void test(darray *, int, int, int, bool);
        benchmark(int, bool);
        ~benchmark();
};
...
void benchmark::test(darray * tablica, int initial,int steps, int repetitions, bool Debug_Mode)
    ...
    for(i=0; i<steps; i++)
    {
        ...
        for(j=0;j<repetitions;j++)
        {
            ...
            for(k=0; k<amount_of_data; k++)
            {
                ...
                tablica->append(data[k], Debug_Mode); //przypisanie okreslonej ilosci danych do listy
            }
            ...
            delete tablica;
            tablica = new darray(amount_of_data, Debug_Mode);
