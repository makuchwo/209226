...
class benchmark
{
    ...
    public:
        ...
        void test(queue *, int, int, int, bool);
        benchmark(int, bool);
        ~benchmark();
};
...
void benchmark::test(queue * kolejka, int initial,int steps, int repetitions, bool Debug_Mode)
    ...
    for(i=0; i<steps; i++)
    {
        ...
        for(j=0;j<repetitions;j++)
        {
            ...
            for(k=0; k<amount_of_data; k++)
            {
                ...
                kolejka->append(tmp); //przypisanie okreslonej ilosci danych do listy
            }
